  ...
  const slides = headings.map((heading, index) => {
    ...
    if (kind === 'title') {
      ...
      const pipelineSql = codes[2].value.split('\n').map(line => {
        const parameter = line.match(/^(\s*)(source|steps|trigger|sink)(\s*=>.*)$/)
        if (!parameter) return escape(line)
        return `${escape(parameter[1])}<mark class="pipeline-interface pipeline-${parameter[2]}">${parameter[2]}</mark>${escape(parameter[3])}`
      }).join('\n')
      className = 'pipeline-slide technical-slide'
      body = `${header}<div class="technical-intro">${html([paragraphs[0]])}</div><div class="pipeline-flow">${rows.map((row, step) => `<div><span class="pipeline-number">0${step + 1}</span><h3>${escape(text(row.children[0]))}</h3>${html([row.children[1]])}${step === 1 ? '<div class="pipeline-gate">신규·입력 변경 → job<br>변경 없음 → 종료</div>' : ''}</div>`).join('')}</div><div class="pipeline-details"><div class="pipeline-rules"><h3>변경 대상과 검색 시점</h3>${html(codes.slice(0, 2))}<div class="technical-note">${html([paragraphs[1]])}</div></div><div class="pipeline-declaration"><h3><code>create_pipeline</code> 인터페이스 <span>%s는 실행 시 바인딩 값</span></h3><div class="pipeline-interface-map"><div class="pipeline-source"><strong>source</strong><span>job table</span></div><b aria-hidden="true">→</b><div class="pipeline-steps"><strong>steps</strong><span>chunk 포함 가능</span></div><b aria-hidden="true">→</b><div class="pipeline-trigger"><strong>trigger</strong><span>on_change</span></div><b aria-hidden="true">→</b><div class="pipeline-sink"><strong>sink</strong><span>embedding table</span></div></div><pre><code class="language-sql">${pipelineSql}</code></pre></div></div>`
...
.pipeline-declaration pre{tab-size:2}.pipeline-interface{padding:1px 5px;border-radius:3px;font-weight:700;color:#fff}.pipeline-source{background:var(--blue)}.pipeline-steps{background:var(--amber);color:var(--navy)}.pipeline-trigger{background:var(--green)}.pipeline-sink{background:var(--coral)}
.pipeline-slide .pipeline-flow{margin-top:20px}.pipeline-slide .pipeline-flow>div{min-height:185px;padding-top:14px}.pipeline-slide .pipeline-number{font-size:26px;margin-bottom:8px}.pipeline-slide .pipeline-flow h3{font-size:25px;margin-bottom:10px}.pipeline-slide .pipeline-flow p{font-size:23px;line-height:1.4}.pipeline-slide .pipeline-gate{font-size:20px;margin-top:10px}.pipeline-slide .pipeline-details{grid-template-columns:550px minmax(0,1fr);gap:44px;margin-top:24px}.pipeline-rules pre{background:transparent;color:var(--navy);padding:0;border:0;box-shadow:none;font-size:24px;line-height:1.4}.pipeline-rules pre+pre{margin-top:22px}.pipeline-rules .technical-note{font-size:22px;line-height:1.45;margin-top:22px}.pipeline-declaration h3{display:block}.pipeline-declaration h3 span{float:right;font-size:19px;color:var(--muted);font-weight:400;white-space:nowrap}.pipeline-declaration pre{font-family:"Cascadia Mono",Consolas,monospace;font-size:22px;line-height:1.15;padding:16px;white-space:pre;overflow-wrap:normal;font-weight:400}
.pipeline-interface-map{display:grid;grid-template-columns:1fr 24px 1.2fr 24px 1fr 24px 1.15fr;align-items:stretch;margin:0 0 12px}.pipeline-interface-map>div{display:grid;gap:3px;padding:8px 12px;border-top:5px solid currentColor;background:#fff}.pipeline-interface-map strong{font:700 20px/1.15 "Cascadia Mono",Consolas,monospace}.pipeline-interface-map span{color:var(--muted);font-size:17px;line-height:1.2;white-space:nowrap}.pipeline-interface-map>b{display:grid;place-items:center;color:var(--muted);font-size:20px}.pipeline-interface-map .pipeline-source{color:var(--blue)}.pipeline-interface-map .pipeline-steps{color:var(--amber)}.pipeline-interface-map .pipeline-trigger{color:var(--green)}.pipeline-interface-map .pipeline-sink{color:var(--coral)}
